...
class Order(db.Model):
    ...
    def __init__(self, orderId, name, symbol, price, orderType, quantityData, stopData, orderStatusData, enteredTime, closeTime, assetType):
        ...
        self.quantity = quantityData['quantity']
        self.filled_quantity = quantityData['filledQuantity']
        self.remaining_quantity = quantityData['remainingQuantity']

        if stopData != None:
            self.stop_price = stopData['price']
            self.stop_price_link_basis = stopData['priceLinkBasis']
            self.stop_price_link_time = stopData['priceLinkTime']
            self.stop_price_offset = stopData['priceOffset']
            self.stop_type = stopData['type']
        else:
            self.stop_price = None
            self.stop_price_link_basis = None
            self.stop_price_link_time = None
            self.stop_price_offset = None
            self.stop_type = None

        self.entered_time = enteredTime
...
{
session
duration: day, good til cancel, fill or kill
orderType: market, limit, stop, stop limit, trailing stop, market on close, exercise, trailing stop limit, net debit, net credit, net zero
